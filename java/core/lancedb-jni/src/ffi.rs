...
use jni::objects::{JByteBuffer, JList, JObjectArray, JString, JValue};
...
pub trait JNIEnvExt {
    ...
    // /// Get Java List<String> from Vec<String>.
    // fn get_java_string_list(&mut self, list: Vec<String>) -> Result<JObject>;
...
impl JNIEnvExt for JNIEnv<'_> {
    ...
    // fn get_java_string_list(&mut self, rust_list: Vec<String>) -> Result<JObject> {
    //     let list_class = self.find_class("java/util/ArrayList")?;
    //     let list_obj = self.alloc_object(list_class)?;
    //     self.call_method(&list_obj, "<init>", "()V", &[])?;
    //     for item in rust_list {
    //         let item_jobj = JObject::from(self.new_string(item)?);
    //         let item_gen = JValue::Object(&item_jobj);
    //         self.call_method(&list_obj, "add", "(Ljava/lang/Object;)Z", &[item_gen])?;
    //     };
    //     Ok(list_obj)
    // }
